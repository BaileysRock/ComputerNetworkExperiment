...
const int SEND_WIND_SIZE = 5;//发送窗口大小为 5，GBN 中应满足 W + 1 <=N（W 为发送窗口大小，N 为序列号个数）
//本例取序列号 0...9 共 10 个
//如果将窗口大小设为 1，则为停-等协议
const int SEQ_SIZE = 10; //序列号的个数，从 0~9 共计 10 个
...
BOOL lossInLossRatio(float ACKLoss){
    int lossBound = (int) (ACKLoss * 100); // 默认的lossRatio = 0.2 lossBound = 20
    int r = rand() % 101;
    if (r <= lossBound){
        return TRUE;
    }
...
void testGBNFile(SOCKET SendSocket, sockaddr_in RecvAddr) {
    ...
    int flag = 0;
    int count = 0;
    int totalcount = (int)(1.0/packetLoss);
    while (true) {
...
//                 模拟ack的丢失情况
                bool b = lossInLossRatio(ACKLoss);
                if (b){
                    cout << "seq of "<<seq<<"is loss!" << endl;
...
unsigned int __stdcall ChildThread(PVOID PM) {
    char buffer[1026*113] = {0};
    int recvSize;
    int MAX_URL_LENGTH = 1026;
    cout << "当前线程ID=" << GetCurrentThreadId() << endl;
    if ((recvSize = recv(*(SOCKET *) PM, buffer, 1026, 0)) != -1) {
...
void SR(SOCKET SendSocket, sockaddr_in RecvAddr) {
    int BufLen = 1026;//缓冲区大小
    char *SendBuf = new char[BufLen];//发送数据的缓冲区
    char *RecvBuf = new char[BufLen];
    int RecvLen = sizeof(SOCKADDR);
