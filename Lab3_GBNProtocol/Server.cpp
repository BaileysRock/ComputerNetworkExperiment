...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        if (strcmp(buffer, "-time") == 0) {
        ...
        else if(strcmp(buffer, "-testgbnSendFile") == 0) {
            ...
            SendBuf[0]=100;
            SendBuf[1] = '\0';
            sendto(sockServer, SendBuf,2, 0, (SOCKADDR *) &addrClient, sizeof(SOCKADDR));
            int recvSeq;
            int waitSeq;
            int stage = 0;
            unsigned seq;
            ...
            while (true) {
                ...
                // 从服务器端读东西 读到buffer中
                int recvSize = recvfrom(sockServer, RecvBuf, BufLen, 0, ((SOCKADDR *) &addrClient), &length);
                if (recvSize == -1)
                    break;
